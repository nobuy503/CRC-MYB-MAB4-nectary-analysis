# Exploratory cluster-marker heatmaps for the integrated snRNA-seq dataset
#
# Input:
#   data/processed/integrated_data.rds
#
# Analyses included:
# - FindAllMarkers across Seurat clusters 0-21 using the Wilcoxon test.
# - Selection and export of the top 100 marker genes per cluster.
# - Per-cell Seurat DoHeatmap plots using top 100 or top 30 markers.
# - Cluster-average, row-Z-scored pheatmap visualization.
# - Alternative ordering of cells by cluster-specific module scores.
#
# Figure relationship:
# This script explores cluster-marker heatmaps and supports the marker tables
# associated with Supplementary Data 1. It is not the code for Fig. 1d, which
# shows the top 100 differentially expressed genes between Flower and Silique.
#
# Provenance note:
# This is a source/exploratory script containing several alternative plotting
# approaches and repeated object-preparation steps.
...
png("results/cluster_marker_heatmaps/heatmap_top100_markers_per_cluster_downsampled.png",
...
write.csv(out_all,
          file = "results/cluster_marker_heatmaps/markers_all_clusters_full.csv",
          row.names = FALSE)
# 圧縮版（任意）
# write.csv(out_all, gzfile("results/cluster_marker_heatmaps/markers_all_clusters_full.csv.gz"), row.names = FALSE)
...
write.csv(top100_by_cluster,
          file = "results/cluster_marker_heatmaps/markers_top100_per_cluster.csv",
          row.names = FALSE)

# --- 4) 遺伝子名だけの一覧も（ヒートマップ用 features_for_heatmap と一致）
features_for_heatmap <- unique(top100_by_cluster$gene)
write.table(features_for_heatmap,
            file = "results/cluster_marker_heatmaps/features_for_heatmap_top100_per_cluster.txt",
...
write.csv(by_cluster_compact,
          file = "results/cluster_marker_heatmaps/markers_top100_per_cluster_compact.csv",
          row.names = FALSE)
...
write.csv(table_counts,
          file="results/cluster_marker_heatmaps/markers_top100_counts_per_cluster.csv",
          row.names = FALSE)
...
saveWorkbook(wb, "results/cluster_marker_heatmaps/markers_top100_per_cluster.xlsx", overwrite = TRUE)
...
# 出力（PNG推奨）
png("results/cluster_marker_heatmaps/heatmap_top30_markers_per_cluster_downsampled.png",
...
pheatmap(
  mat_z_ord,
  filename = "results/cluster_marker_heatmaps/heatmap_top100_clusterAverages_ordered.pdf",
...
# ファイルへ直書き（重い描画を安定化）
png("results/cluster_marker_heatmaps/heatmap_top100_percluster_ordered_percell.png",
    width = 4200, height = 3200, res = 300)
p_hm <- DoHeatmap(
  obj,
