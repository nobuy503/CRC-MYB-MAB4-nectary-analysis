# Source script for flower snRNA-seq visualizations in Supplementary Figure 1
#
# This file preserves the analysis code supplied by the authors.
# Dataset: flower portion of NCBI GEO GSE226097.
#
# Note: this source version assumes that the Seurat object
# GSE226097_flower_230221 already exists in the R environment. It includes
# exploratory plotting commands as well as code used for the figure. A
# streamlined reproducible version will be prepared after the final panel(s)
# and object-preparation steps are confirmed.
...
#UMAPを描くだけ
class(GSE226097_flower_230221) 
p1 <- DimPlot(GSE226097_flower_230221)
print(p1)

#クラスターの順にUMAPを描くだけ
# オブジェクトのクラスを確認
object_class <- class(GSE226097_flower_230221)
cat("Class of the object:", object_class, "\n")
# クラスターの順序を確認
current_levels <- levels(Idents(GSE226097_flower_230221))
cat("Current cluster levels:", current_levels, "\n")
# 正しい順序に並べ替える（例: 数値順）
desired_order <- sort(as.numeric(as.character(current_levels)))
Idents(GSE226097_flower_230221) <- factor(Idents(GSE226097_flower_230221), levels = as.character(desired_order))
# DimPlotを作成して表示
p1 <- DimPlot(GSE226097_flower_230221)
print(p1)

#クラスターの順にUMAPを描くだけ+クラスター名を表示# meta.data内の列名を確認
# meta.data内の列名を確認
colnames(GSE226097_flower_230221@meta.data)
p1 <- DimPlot(GSE226097_flower_230221, group.by = "seurat_clusters", label = T)
print(p1)
...
#普通にFeaturePlotを描く
FeaturePlot(GSE226097_flower_230221, features = c("AT1G69180", "AT4G31820", "AT2G39060"), ncol = 3) 

#陽性細胞を全面に出してにFeaturePlotを描く
FeaturePlot(GSE226097_flower_230221, features = c("AT1G69180", "AT4G31820", "AT2G39060"), ncol = 3, order =T) 

# 凝ったFeaturePlotを作成 1
FeaturePlot(
  GSE226097_flower_230221, 
...
#普通にdot plotを描く
DefaultAssay(GSE226097_flower_230221) <- "RNA"　#RNAで指定が必要
DotPlot(GSE226097_flower_230221, features = c("AT1G69180", "AT4G31820", "AT2G39060", "AT3G01530", "AT3G27810", "AT5G40350", "AT5G20830", "AT2G26580", "AT1G77110", "AT2G36190"), dot.scale = 20) 

#dot plotを描く、gene mappingとラベルを斜めに、Y軸ラベルを直接編集
DefaultAssay(GSE226097_flower_230221) <- "RNA"　#RNAで指定が必要
# DotPlotを作成してオブジェクトとして保存
p <- DotPlot(GSE226097_flower_230221, 
             features = c("AT1G69180", "AT4G31820", "AT2G39060", 
...
# 普通にVlnplotを表示
VlnPlot(GSE226097_flower_230221,
        features = c("AT1G69180", "AT4G31820", "AT2G39060", 
                     "AT3G01530", "AT3G27810", "AT5G40350", 
                     "AT5G20830", "AT2G26580", "AT1G77110", 
                     "AT2G36190"),
...
VlnPlot(GSE226097_flower_230221,
        features = c("AT1G69180", "AT4G31820", "AT2G39060", 
                     "AT3G01530", "AT3G27810", "AT5G40350", 
                     "AT5G20830", "AT2G26580", "AT1G77110", 
                     "AT2G36190", "AT4G36220", "AT1G51680", "AT2G26750", "AT2G27770", "AT1G01490",
...
# 普通にHeatmapを表示
# スケーリングを実行
GSE226097_flower_230221 <- ScaleData(GSE226097_flower_230221, features = rownames(GSE226097_flower_230221))
DoHeatmap(GSE226097_flower_230221, features = c("AT1G69180", "AT4G31820", "AT2G39060", 
                                                "AT3G01530", "AT3G27810", "AT5G40350", 
                                                "AT5G20830", "AT2G26580", "AT1G77110", 
                                                "AT2G36190"))
# スロット値を表示, 表示域を変更　
